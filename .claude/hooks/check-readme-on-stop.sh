...
cat <<'EOF'
{"decision": "block", "reason": "README SYNC — Du code a été modifié pendant cette session. Vérifie si README.md et CONTRIBUTING.md (section Architecture) doivent être mis à jour (nouvelles commandes, options modifiées, workflow changé, sections CI/CD, distinction static demo / application). Si la doc est déjà à jour, supprime le marker avec : rm .claude/.pending-readme-check"}
EOF
